function [A, C] = computeCurvature(SK, S, ws)
...
% (keep it or not ?)
% [a virer pour retrouver les anciens resultats ne pas oublier de corriger
% l'intervalle de calcul de courbure] 
A(1:ws) = A(ws+1);            
...
% [ca de meme pour avoir les points sur laquelle on a pas de courbure]
C(1:ws) = (1:ws).*C(ws+1) / ws; 
% [et ca pour etre tranquille]
C(n-ws:end) = (ws-(1:ws+1)) .* C(end-ws-1) / ws; 

% keep it or not ?
% [et puis on degagera ca aussi]
C = moving_average(C, ceil(ws/2)); 
